...
import { FaCode, FaEnvelope, FaGraduationCap, FaHome, FaProjectDiagram, FaBars, FaTimes } from "react-icons/fa"
    ...
    <>
      ...
      >
        <div className="bg-[#112836] w-auto max-w-[200px] xs:max-w-[240px] sm:max-w-sm md:max-w-[900px] px-1 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-3 rounded-full border border-[#38bdf8] sm:border-2 shadow-lg relative">
          ...
          <button
            className="md:hidden absolute left-1.5 top-1/2 transform -translate-y-1/2 text-white p-1"
            onClick={toggleMenu}
          ...
          <ul className="md:hidden flex justify-center items-center text-white gap-1 xs:gap-2 sm:gap-3 pl-6">
            {navItems.map((item, index) => (
              ...
              >
                <a href={item.href} className="flex items-center" aria-label={item.text}>
                  <span className="text-xs xs:text-sm sm:text-base">{item.icon}</span>
                </a>
              </li>
            ))}
          </ul>
      ...
      {/* Mobile Menu Overlay */}
      {isMenuOpen && (
        <div className="fixed inset-0 z-40 bg-black bg-opacity-50 md:hidden" onClick={closeMenu}>
          <div
          ...
          >
            <ul className="flex flex-col space-y-1 text-white">
              {navItems.map((item, index) => (
                <li key={index} className="transition-all duration-300 hover:bg-[#38bdf8] hover:text-white rounded-lg">
                  <a href={item.href} className="flex items-center p-2.5" onClick={closeMenu}>
